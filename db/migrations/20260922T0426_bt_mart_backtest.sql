...
-- mart_readonly select access: no migration file grants per-table (grep across
-- db/migrations/*.sql confirms it — this repo's own convention). db/roles.sql's
-- `grant select on all tables in schema mart to mart_readonly` plus its `alter
-- default privileges` already cover every table in this schema, present and
-- future, and roles.sql re-applies after every migration file on every boot
-- (db/apply_migrations.sh, Makefile db-migrate). A per-table grant HERE would
-- run before that role exists on a role-less cluster (CI's ephemeral Postgres
-- has none until roles.sql runs) and fail migration apply outright.
