-- #877 PR-2 (second half): fill the entity identity store from what is already stored.
--
-- The store and its rules are the previous migration (…T0412_datahub_entity_identity_store);
-- the design is docs/entity-identity.md. This file holds only the backfill:
--
--   staging.entity_backfill_plan   the whole plan as one SELECT over tables that existed
--                                  before either migration, so it can be run read-only
--                                  against an environment before deploy
--   staging.entity_backfill()      applies the plan; idempotent; called on every boot below
--
-- Ids are derived (staging.entity_mint): the same stored evidence mints the same UUIDs in
-- every database. Until PR-3 switches captures to UUIDs, every boot gives new legacy ids an
-- entity.
...
select staging.entity_backfill() as entity_backfill;
