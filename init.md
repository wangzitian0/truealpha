...
| every external call is gated (rule 6) | Production's `staging.api_call_ledger` (the schema name is the medallion layer, not the environment) carries 2,186 rows across seven sources — SEC, Twelve Data, yahoo, OpenFIGI, N-PORT, the Nasdaq index and `filing-extraction-model`. moomoo is declared but has never written a row, which is why question 4 has no evidence rather than no capacity | #729 |
