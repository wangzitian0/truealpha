...
25. **The Polars AST is the selected factor-expression engine and VectorBT is the selected backtest engine; neither is a data or semantic authority.** Serializable TrueAlpha definitions are versioned `truealpha_contracts.ast` expressions compiled through `factors.expressions`, and only adapters inside `libs/factors` may invoke either engine. Both receive provenance-neutral inputs projected from durable PIT snapshots and the explicit post-decision market-event stream; neither may crawl or select vintages, resolve membership, infer confidence or lineage, combine adjusted prices with explicit actions, or replace Decimal monetary logic. Every run pins the engine build, adapter, strategy, and input snapshot identities through `ExpressionEngineExecutionBinding`. Pinning the operator set itself has no mechanism yet and is owned by the backtest runner (#26); A5 records why the Qlib operator-registry digest was not carried across rather than reproduced as a placeholder. Native `libs/factors` implementations remain the source of truth for Decimal monetary results, and stay valid for graph and other computations that do not fit a columnar matrix-expression model; a compiled expression is a reproducibility cross-check of that Decimal result, never its replacement. (Qlib held this role until 2026-09-23; see `docs/architecture-decisions/A5-polars-vectorbt-engine.md`.)
