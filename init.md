...
| moomoo | Rate-limited (bursts/30s), not a monthly quota — see below | **Never called (2026-09-10):** `staging.api_call_ledger` holds calls from sec, twelvedata, yahoo, filing-extraction-model, openfigi, nport and nasdaq-index — and zero from moomoo, ever; `staging.analyst_rating_events` has the right columns and no rows (#771). Historical per-analyst rating depth is confirmed as a vendor CAPABILITY, not as a path this system has exercised. Historical public availability for backtest eligibility still requires independent evidence; vendor backfill/update time is not a substitute. ETF weights and company-level supply-chain edges are not sourced from moomoo. |
