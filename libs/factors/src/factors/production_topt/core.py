...
class ToptCoreSnapshotInput(_FrozenModel):
    """Exact one-instrument projection from a persisted TOPT snapshot."""
    ...
    @classmethod
    def validate_observation_ids(cls, values: tuple[str, str, str, str]) -> tuple[str, str, str, str]:
        if len(set(values)) != 4 or tuple(sorted(values)) != values:
            raise ValueError("snapshot observation IDs must contain four sorted unique cells")
...
class ToptCoreResult(_FrozenModel):
    ...
    input_observation_ids: tuple[str, str, str, str]
    ...
    def validate_and_identify(self) -> Self:
        ...
        if self.availability is ToptCoreAvailability.AVAILABLE:
            if any(value is None for value in numeric_values) or self.tier is None or self.reason_codes:
                raise ValueError("available TOPT core result requires all values and no reason codes")
        elif any(value is not None for value in numeric_values) or self.tier is not None or not self.reason_codes:
            raise ValueError("unavailable TOPT core result requires reasons and no computed values")
        for field_name in (
            "capital_adjusted_gross_profit",
            "gppe",
            "target_ps_lower",
...
def compute_topt_core(
    ...
    *,
    ...
    """Compute one instrument result without source, storage, or provenance branching."""
    ...
    inputs = {
        ...
        ToptCoreReasonCode.MISSING_SHARES_OUTSTANDING: snapshot.shares_outstanding,
        ToptCoreReasonCode.MISSING_MARKET_PRICE: snapshot.market_price,
    }
    reasons = tuple(
        reason
    ...
    present = tuple(metric for metric in inputs.values() if metric is not None)
    freshness = (
        MetricFreshness.STALE
        if any(metric.freshness is MetricFreshness.STALE for metric in present)
        else MetricFreshness.UNKNOWN
        if any(metric.freshness is MetricFreshness.UNKNOWN for metric in present)
    ...
    assert snapshot.shares_outstanding is not None and snapshot.shares_outstanding.value is not None
    assert snapshot.market_price is not None and snapshot.market_price.value is not None
    ...
    with localcontext(_DECIMAL_CONTEXT):
        ...
        market_cap = snapshot.market_price.value * snapshot.shares_outstanding.value
    ...
    confidence = min(metric.confidence for metric in present)
    return ToptCoreResult(
        invocation_id=invocation_id,
        snapshot_id=snapshot.snapshot_id,
        run_id=snapshot.run_id,
