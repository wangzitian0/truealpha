...
def compile_factor_panel(
    ...
    Parameters:
    - factor_expr: AST node or raw polars.Expr or string column name.
    ...
    - pl.DataFrame with calculated 'factor_value' and prior sorting on ['symbol', 'date'].
      If mask_df is provided, missing or ineligible entries have factor_value set to None (Fail-Closed).
    """
    ...
    if mask_df is not None:
        ...
        if "eligible" in pdf.columns:
            # Strictly Fail-Closed: missing mask row is ineligible (False)
...
def compute_topk_dropout_weights(
    ...
    """Evaluate cutoffs chronologically and calculate Top-K Dropout target weights.

    - Evaluates cutoffs chronologically.
    - Applies dynamic universe mask where eligible == True (Fail-Closed: missing mask row is ineligible).
    - Top-K selection with dropout_k turnover limit.
    - Ineligible assets are dropped immediately regardless of dropout_k.
    - Assets with NaN or null factor values are ineligible.
    ...
    has_mask = mask_df is not None or "eligible" in fdf.columns

    # Merge external mask if provided
    if mask_df is not None:
        mdf = _to_polars(mask_df)
        m_date_col = _find_date_col(mdf.columns)
        join_cols = [c for c in ["symbol", m_date_col, "eligible"] if c in mdf.columns]
        fdf = fdf.join(
            mdf.select(join_cols),
            left_on=["symbol", cutoff_col],
            right_on=["symbol", m_date_col],
            how="left",
        )
        # Fail-closed: missing mask entry is False
        fdf = fdf.with_columns(pl.col("eligible").fill_null(False).cast(pl.Boolean))
    ...
    for c in cutoffs:
        ...
        # Identify eligible candidates with valid factor values
        candidates: list[tuple[str, float]] = []
        for r in rows:
            sym = str(r["symbol"])
            val = r.get(factor_val_col)
            if has_mask:
                raw_eligible = r.get("eligible")
                eligible = bool(raw_eligible) if raw_eligible is not None else False
            else:
                eligible = True

            # Candidate must have eligible is True and non-null, non-nan factor value
            if eligible is True and val is not None and not (isinstance(val, float) and np.isnan(val)):
