...
def _fact(
    ...
    *,
    ...
    return Fact(
        ...
        metric=metric,
        value=value,
        unit_family=unit_family or _DEFAULT_UNIT_FAMILY[metric],
...
def test_wrong_unit_family_is_rejected_not_silently_combined():
    facts = [
        _fact("gross_profit", "1000000", unit_family=UnitFamily.RATIO),
        _fact("total_assets", "4000000"),
        _fact("employees_total", "100"),
    ]
    result = gross_profit_per_employee(facts, entity_id="issuer.acme", as_of=_AS_OF, risk_free_rate=_RISK_FREE_RATE)
    assert result.value is None
    assert "unexpected_unit_family_gross_profit" in result.flags
