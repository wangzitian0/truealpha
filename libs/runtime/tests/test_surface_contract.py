...
def test_the_probe_identifies_itself_to_the_edge() -> None:
    """Measured: the edge answers urllib's default `Python-urllib/3.x` with
    403 while curl gets 200 for the same URL. Without an explicit User-Agent
    the daily probe reports the surface down every day, and a check that cries
    wolf is worse than no check at all."""
    source = (REPO_ROOT / "tools" / "surface_contract.py").read_text(encoding="utf-8")
    assert 'headers={"User-Agent": USER_AGENT}' in source, "the probe no longer identifies itself; the edge returns 403"
