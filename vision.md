...
- **The "large-model-driven company" three-tier framework**: whether a company needs to add headcount proportionally to handle a new category of decisions. Companies are sorted into three tiers, each mapping to a different P/S multiple range (traditional 3-4x / tech 8-10x / large-model-native 20-30x). This is a genuine computed output (init.md module 7), derived from gross profit per employee — not just a mental model for reading that number
