 ...
 * and nothing else). This turns the cutoff into an age and a verdict, with the SAME
 * threshold `tools/output_invariants.py`'s `pointer-has-advanced-recently` uses — one
 * daily cycle plus slack, 36 hours — so the page and the nightly invariant cannot
...
export interface ServedRunAge {
	...
	label: string;
	/** True past one scheduled cycle plus slack — the amber the invariant would raise. */
	stale: boolean;
}
