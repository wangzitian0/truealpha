...
/** Next.js does not decode a dynamic route segment for us — params.id may still
 * be percent-encoded. Malformed percent-encoding must not 500 the route —
 * same guard as research/entities/[id] and research/library/[id]. */
function decodeRunId(id: string): string {
  try {
...
export default async function BacktestDetailPage({ params }: PageProps) {
  ...
  const id = decodeRunId(resolvedParams.id);
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <div className="mt-1 text-2xl font-bold font-mono text-emerald-400">
            {formatPercentFromFraction(run.cagr_monthly) ?? "—"}
          </div>
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <div className="mt-1 text-2xl font-bold font-mono text-rose-400">
            {formatPercentFromFraction(run.max_dd_daily) ?? "—"}
          </div>
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <div className="mt-1 text-2xl font-bold font-mono text-amber-400">
            {formatPercentFromFraction(run.vol_daily) ?? "—"}
          </div>
                ...
                trades.map((t) => (
                  <tr key={t.trade_id} className="hover:bg-muted/20 font-mono">
                    ...
                    <td className="px-4 py-2 text-right">
                      {t.shares !== null ? parseFloat(t.shares).toFixed(2) : "—"}
                    </td>
                    <td className="px-4 py-2 text-right">
                      {t.execution_price !== null ? `$${parseFloat(t.execution_price).toFixed(2)}` : "—"}
                    </td>
                    <td className="px-4 py-2 text-right">
                      {t.trade_value !== null ? `$${parseFloat(t.trade_value).toLocaleString()}` : "—"}
                    </td>
                    <td className="px-4 py-2 text-right">
                      {t.weight_after !== null ? `${(parseFloat(t.weight_after) * 100).toFixed(1)}%` : "—"}
                    </td>
                    <td className="px-4 py-2 text-right text-gray-500">
                      {t.fee_paid !== null ? `$${parseFloat(t.fee_paid).toFixed(2)}` : "—"}
                    </td>
                  </tr>
                ))
              )}
