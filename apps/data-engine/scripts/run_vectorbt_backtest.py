#!/usr/bin/env python3
"""CLI operator script: run a VectorBT dual-resolution Top-K backtest from staging
market data and the PIT universe mask, and persist the result to `mart.backtest_*`
(#101 datahub staging tables, #102 factor expression compiler + adapter, #103
VectorBT engine + mart persistence).

Chains every layer this stack added, end to end, against real database rows —
this is the deployed path that makes `factors.expressions`, `factors.backtest`,
and `data_engine.datahub.{market_prices,universe_mask}` reachable, not just a
place they are imported for show:
    ...
    staging.market_prices_{daily,monthly} + staging.universe_mask   (ingested by
      ...
      -> factors.backtest.adapter              (Top-K Dropout target weights,
                                                 ...
                                                 wide matrices VectorBT wants)
      -> factors.backtest.engine               (dual-resolution simulation)
      -> factors.backtest.storage              (mart.backtest_runs / _valuations /
                                                 _trades)

The factor expression is a cross-sectional rank of whichever price column
`--factor-column` names (`close` by default) — a real, computable expression
exercised the same way `libs/factors/tests/backtest/test_adapter.py`'s
end-to-end test does, not a business strategy choice this script is making on
an operator's behalf. Every other input (symbols, universe id, strategy
identity, top-k/dropout-k) is a required or explicit CLI argument.
...
def _load_prices(connection: psycopg.Connection, table: str, symbols: list[str]) -> pl.DataFrame:
    query = f"select symbol, date, close from {table} where symbol = any(%s) order by date asc"  # noqa: S608 - table is one of two fixed literals below, never user input
...
def run_backtest(
    ...
    engine = VectorBTBacktestEngine(BacktestEngineConfig())
    result = engine.run(
