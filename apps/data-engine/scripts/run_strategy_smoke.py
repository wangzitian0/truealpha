...
- There is no BacktestDataGateway, DecisionSnapshot/ReplayEventStream,
  Dagster asset, or persisted StrategyRun/Trade/PortfolioValuation record.
  Those remain real, tracked gaps toward #26's full acceptance.
- Quantization (labor-efficiency/P-S/valuation-gap decimal places) is
  ...
  quantization rules at the same assembly point this script does.

Usage: uv run --package truealpha-data-engine python \
    apps/data-engine/scripts/run_strategy_smoke.py
"""
...
def _rank_and_select(decisions: list[Decision], selection_count: int) -> list[Decision]:
    ...
    for cutoff_at, group in by_cutoff.items():
        ...
        rankable.sort(key=lambda item: (-item.valuation_gap, item.issuer_id))
